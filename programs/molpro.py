...
def check_input(args,nodeInfo):
  ...
  if args['parseInput']:
    ...
    nodeMemLimit = nodeInfo[args['queue']]['nodeMem'] * 131072 * math.pow(10,-6) * 0.95 / args['nslot']
    if memMatch:
      ...
      if jobMemory > nodeMemLimit:
        error('Please reduce the memory of your job from '+str(int(jobMemory))+' m to '+str(int(nodeMemLimit))+' m or less for '+str(args['nslot'])+' processors')
    else:
